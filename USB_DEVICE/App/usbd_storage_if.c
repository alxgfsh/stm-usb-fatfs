...
int8_t SD_IsDetected(void)
{
  /* Assuming SD_DETECT_Pin is active low (0 when card is inserted) */
  if (HAL_GPIO_ReadPin(SD_DETECT_GPIO_Port, SD_DETECT_Pin) == GPIO_PIN_RESET) {
    return 1; /* Card detected */
  }
  return 0; /* Card not detected */
}
...
int8_t STORAGE_IsReady_FS(uint8_t lun)
{
  ...
  HAL_SD_CardStateTypeDef cardState = HAL_SD_GetCardState(&hsd);
    ...
    if (cardState == HAL_SD_CARD_TRANSFER ||
        ...
        cardState == HAL_SD_CARD_RECEIVING) {
      return (USBD_OK);
    }

    /* Try to reinitialize if card state is bad */
    if (cardState == HAL_SD_CARD_ERROR ||
        cardState == HAL_SD_CARD_DISCONNECTED) {
      HAL_SD_DeInit(&hsd);
